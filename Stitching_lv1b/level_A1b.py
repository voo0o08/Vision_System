# lebel_A1b 이미지 순서가 정해짐 (1,1)->(1,2)->(2,1)->(2,2)
from tkinter import *
...
for i in range(len(img)-1): #i=0,1,2
    ...
    right = I_1
    left = I0  
    # 위치 이동에 음수가 있다면 두 이미지를 바꾸어 준다 -> change 함수
    ...
    height = left.shape[0]+right.shape[0]

    dst=cv2.warpPerspective(right, matrix, (width, height), borderValue=0)
    
    #이미지를 크기에 맞게 자르기 위한 코드
    if mov_row+10<abs(round_M[1,2]):
        mov_row=abs(round_M[1,2])
        total_row=total_row+abs(round_M[1,2])
    if mov_col+10<abs(round_M[0,2]):
    ...
    #print("dst=",dst.shape,"left=",left.shape)
